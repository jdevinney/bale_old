...
alloc_buffers(twohop_t* twohop)
{
  ...
  size_t row_size = twohop->n_cols * twohop->row_quads * 4;
  ...
  size_t col_size = twohop->n_rows * twohop->col_quads * 4;
...
static void
reset_send_areas(size_t n, area_t areas[n], uint32_t* base, size_t quads)
{
  for (size_t i = 0; i < n; i++) {
    areas[i].next = base + i * quads;
    areas[i].limit = areas[i].next + quads;
  }
}

static void
reset_recv_areas(size_t n, area_t areas[n], uint32_t* base, size_t quads)
{
  for (size_t i = 0; i < n; i++) {
    areas[i].next = base + i * quads;
...
{
  const size_t quads = twohop->row_quads;
...
row_advance(twohop_t* twohop)
{
  ...
  const size_t row_quads = twohop->row_quads;
  ...
  // Call the library function
  {
    ...
    shmemx_alltoallv(twohop->row_recv_bufs, twohop->row_offsets, twohop->recv_sizes,
                     ...
                     twohop->row_sync[0]);
    long* temp = twohop->row_sync[0];
    twohop->row_sync[0] = twohop->row_sync[1];
    twohop->row_sync[1] = temp;
#else
...
{
  reset_send_areas(twohop->n_rows, twohop->col_send,
                   twohop->col_send_bufs, twohop->col_quads);
  ...
  // Call the library function
  {
    ...
    if (!twohop->odd) {
      ...
      shmemx_team_alltoallv(twohop->col_recv_bufs, twohop->col_offsets, twohop->recv_sizes,
                            twohop->col_send_bufs, twohop->col_offsets, twohop->send_sizes,
                            twohop->col_team, twohop->col_sync[0]);
    long* temp = twohop->col_sync[0];
    twohop->col_sync[0] = twohop->col_sync[1];
    twohop->col_sync[1] = temp;
#else
    int error = MPI_Alltoall(twohop->send_sizes, 1, MPI_INT,
...
twohop_unpull(convey_t* self)
{
  ...
  uint32_t* bottom = twohop->col_recv_bufs + row * twohop->col_quads;
...
twohop_advance(convey_t* self, bool done)
{
  ...
  if (twohop->col_quiet) {
    ...
    reset_send_areas(twohop->n_cols, twohop->row_send,
                     twohop->row_send_bufs, twohop->row_quads);
    if (result < 0)
      return result;
...
{
  reset_send_areas(twohop->n_rows, twohop->col_send, twohop->col_send_bufs, twohop->col_quads);
  reset_recv_areas(twohop->n_rows, twohop->col_recv, twohop->col_recv_bufs, twohop->col_quads);
  reset_send_areas(twohop->n_cols, twohop->row_send, twohop->row_send_bufs, twohop->row_quads);
  reset_recv_areas(twohop->n_cols, twohop->row_recv, twohop->row_recv_bufs, twohop->row_quads);
}

static int
twohop_begin(convey_t* self)
{
  twohop_t* twohop = (twohop_t*) self;
                  ...
                  const convey_alc8r_t* alloc, uint64_t options)
{
  ...
  if (capacity == 0 || item_size == 0 || row_procs == 0)
  ...
  size_t row_quads = packet_quads * (wide ? capacity : (capacity * n_rows) / n_cols);
  size_t col_quads = packet_quads * (wide ? (capacity * n_cols) / n_rows : capacity);
  ...
  *twohop = (twohop_t) {
  ...
  {
    int error;
    error = MPI_Comm_split(twohop->comm.internal->mpi_comm,
...
#undef GRAB
  ...
  for (size_t c = 0; c < n_cols; c++)
    twohop->row_offsets[c] = c * (4 * twohop->row_quads);
  for (size_t r = 0; r < n_rows; r++)
    twohop->col_offsets[r] = r * (4 * twohop->col_quads);
#ifdef MPP_USE_SHMEM
  for (int i = 0; i < 2; i++)
