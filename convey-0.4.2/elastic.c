...
{
  return convey_new_trivial(item_bytes, monster_bytes, alloc, options);
}
...
typedef struct elastic {
  ...
  uint32_t* backup; // packet containing most recently pulled item
...
monster_unpull(elastic_t* elastic)
{
  ...
  if (buffer == NULL || buffer->start < ticket_quads)
    return convey_FAIL;

  buffer->start -= ticket_quads;
  elastic->unpull = true;
  elastic->backup = NULL;
  return convey_OK;
}
...
elastic_epull(convey_t* self, convey_item_t* result)
{
  ...
  uint32_t* packet = &buffer->data[buffer->start];
  uint32_t descr = packet[1];
  buffer->start += PORTER_QUADS(descr);
  int64_t from = origin_from_tag(tensor, order, packet[0]);
  bool monster = PORTER_TICKET(descr);
  elastic->bigpull = monster;
  elastic->backup = packet;
...
static int
elastic_begin(convey_t* self)
{
  int a = tensor_begin(self);
                   ...
                   const convey_alc8r_t* alloc, uint64_t options)
{
  ...
  // Build the underlying tensor conveyor, without strong alignment
  uint64_t noalign_opts = options - (align - 1) * convey_opt_NOALIGN;
  convey_t* self = convey_new_tensor(capacity, atom_bytes, order, n_local,
                                     n_buffers, alloc, noalign_opts);
  if (self == NULL)
    return NULL;
  ...
  elastic->comm = comm.internal->mpi_comm;
