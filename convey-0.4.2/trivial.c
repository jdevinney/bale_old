...
convey_t*
convey_new_trivial(size_t item_size, size_t monster_size,
                   const convey_alc8r_t* alloc, uint64_t options)
{
  bool quiet = (options & convey_opt_QUIET);
  if (monster_size == 0 || monster_size < item_size)
  ...
  *trivial = (trivial_t) {
    .convey = { ._class_ = &trivial_methods,
                .features = convey_ELASTIC | convey_STEADY,
                .item_size = item_size, .n_procs = 1, .state = convey_DORMANT,
